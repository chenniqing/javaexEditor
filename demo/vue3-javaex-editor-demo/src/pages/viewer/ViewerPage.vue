<template>
  <header class="demo-header">
    ...
    <div class="demo-actions">
      <button type="button" class="button button-primary" @click="renderViewerContent">刷新展示内容</button>
    </div>
  </header>

  <section class="viewer-page">
    <article ref="viewerBody" class="doc-body" v-html="viewerHtml"></article>
  </section>
</template>
...
onMounted(renderViewerContent);

function renderViewerContent() {
  viewerHtml.value = localStorage.getItem(savedContentKey) || getViewerFallbackHtml();
  nextTick(() => {
    enhanceCodeBlocks(viewerBody.value);
    decorateXiguaPlayers(viewerBody.value).catch(() => {});
  });
}
